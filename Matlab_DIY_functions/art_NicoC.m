function varargout = art_NicoC(varargin)
% art - module for automatic and manual detection and removal of outliers.
%
% edits by Nicolas Clairis - june 2016 for automatic selection of files
% lines ~237 - 333 + march 2017 for MotiScan2 study
%
% art
...
if ~isempty(sess_file) % read config
    ...
    %% manual modifications for multiseq START
    which_study = spm_input('Select study.',1,'m',...
             ' sequence comparison (april-may 2016) | MotiScan june 2016 | MotiScan 2 march 2017', ...
             [0 1 2], 0);
         if which_study == 0
             root = 'B:\resultats\multiseq_april_may2016\';
         elseif which_study == 1
             root = 'B:\resultats\MBB_june2016\';
    ...
    %         [0 1 2], 0);
    motionFileType = 0;
    sub = spm_input('Subject_number?',1,'n',1,1);
    date = spm_input('date experiment (xxYYzz)?',1,'n',1,1);
    date_expe = num2str(date);
    if date < 100000 % number doesn't take into account zero at the beginning
        date_expe = ['0',num2str(date)];
    end
    if which_study == 0 || which_study == 2
        subject = ['s',num2str(sub),'_',date_expe];
    elseif which_study == 1 && sub < 10
        subject = ['s0',num2str(sub),'_',date_expe];
    elseif which_study == 1 && sub >= 10
          subject = ['s',num2str(sub),'_',date_expe];  
    end
%     subject = ['s00_',date_expe]; % specific for sub_00
    cd([root,subject,'\fMRI_scans\']);
    subj_scans_folder = pwd;
    
    if which_study == 0
        what_seq = spm_input('Select type of sequence.',1,'m',...
            ' EPI | multiBAND | multiECHOS', ...
            [0 1 2], 0);
        if what_seq == 0
            subj_EPI_scan_folders_names = ls('*TR2010_PA*'); % name for functional runs ('*TR2180_PA' for instance for runs with TR=2.18s)
            nb_EPI = size(subj_EPI_scan_folders_names,1); % number of EPI sequences for this particular subject
        elseif what_seq == 1
            subj_mBAND_scan_folders_names = ls('*TR1200_2_5iso');
            nb_mBAND = size(subj_mBAND_scan_folders_names,1);
        elseif what_seq == 2
            subj_mECHOzero_scan_folders_names = ls('*PA_Echo_0');
            nb_mECHOzero = size(subj_mECHOzero_scan_folders_names,1);
            subj_mECHOone_scan_folders_names = ls('*PA_Echo_1');
            nb_mECHOone = size(subj_mECHOone_scan_folders_names,1);
        end
    elseif which_study == 1
        what_seq = 0; % only EPI
        subj_EPI_scan_folders_names = ls('*TR2010_PA*'); % name for functional runs ('*TR2180_PA' for instance for runs with TR=2.18s)
        nb_EPI = size(subj_EPI_scan_folders_names,1); % number of EPI sequences for this particular subject
    elseif which_study == 2
        what_seq = 3; % only multiBAND but not the same as for first study
        if strcmp(subject,'s1_240217')
            subj_mBAND_MS2_scan_folders_names = ls('*TR1100_2_5iso_PA_RUN*'); % takes all functional runs folders (if TR = 1.10s, for multiband seq in particular)
        else
            subj_mBAND_MS2_scan_folders_names = ls('*TR1100_2iso_PA_RUN*'); % takes all functional runs folders (if TR = 1.10s, for multiband seq in particular)
        end
        % erase REFBLIP runs from the list (made only for Romain topup
        % correction)
        for iRun = size(subj_mBAND_MS2_scan_folders_names,1):-1:1
            ...
            if strcmp(subj_mBAND_MS2_scan_folders_names(iRun,end-7:end),'_REFBLIP') ||...
                    (strcmp(subject,'s1_030317') && strcmp(subj_mBAND_MS2_scan_folders_names(iRun,1:32),'S07_MB3_ep2d_TR1100_2iso_PA_RUN3'))
                subj_mBAND_MS2_scan_folders_names(iRun,:) = [];
            end
            
        end
        nb_mBAND_MS2 = size(subj_mBAND_MS2_scan_folders_names,1);
    end
    
    if what_seq == 0 % EPI
        num_sess = nb_EPI;
        subj_scan_folders_names = cell(num_sess,1);
        for episeq = 1:nb_EPI
            subj_scan_folders_names(episeq,1) = {subj_EPI_scan_folders_names(episeq,:)};
        end
    elseif what_seq == 1 % mBAND
        num_sess = nb_mBAND;
        subj_scan_folders_names = cell(num_sess,1);
        for mBANDseq = 1:nb_mBAND
            subj_scan_folders_names(mBANDseq,1) = {subj_mBAND_scan_folders_names(mBANDseq,:)};
        end
    elseif what_seq == 2 % leads to comparison echo0/echo1
        num_sess = nb_mECHOzero + nb_mECHOone;
        subj_scan_folders_names = cell(num_sess,1);
        for mECHOseq = 1:num_sess/2
            subj_scan_folders_names(2*(mECHOseq-1)+1,1) = { subj_mECHOzero_scan_folders_names(mECHOseq,:) };
            subj_scan_folders_names(2*mECHOseq,1) = { subj_mECHOone_scan_folders_names(mECHOseq,:) };
        end
    elseif what_seq == 3
        num_sess = nb_mBAND_MS2;
        subj_scan_folders_names = cell(num_sess,1);
        for mBAND_MS2_seq = 1:nb_mBAND_MS2
            subj_scan_folders_names(mBAND_MS2_seq,1) = {subj_mBAND_MS2_scan_folders_names(mBAND_MS2_seq,:)};
        end
    end
    ...
    for i = 1:num_sess
        switch spm_ver
            ...
            case {5,8,12}
                ...
                P{i} = spm_select('ExtFPList',pwd,'^rf.*\.img$'); %% manual change
        end

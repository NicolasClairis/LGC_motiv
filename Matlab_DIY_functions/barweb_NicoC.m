function [xpos, handles] = barweb_NicoC(barvalues, errors,...
    width, groupnames, bw_title,...
    bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend, error_sides, legend_type)
% Usage: [xpos, handles] = barweb(barvalues, errors, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend, error_sides, legend_type)
% xpos: to extract x.values of the bars in case you want to add text above
% the bars (like stars for significance) (added by NicoC)
%
...
% Adapted by Nicolas Clairis: December 2016 (ylim commented and added
% extraction of xpos)
...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
	...
	% Plot erros
    xpos = zeros(numgroups,numbars); % NicoC change: x replaced by xpos and extracted: to extract x values of the bars (in case you want to add a star for significance for ex.)
	for iBar = 1:numbars
        if strcmp(version,'8.0.0.783 (R2012b)')
            x = get(get(handles.bars(iBar),'children'), 'xdata'); % does not work in Matlab 2017
            xpos(:,iBar) = mean(x([1 3],:));
        elseif strcmp(version,'9.2.0.556344 (R2017a)')
            x = [bsxfun(@plus, handles.bars(iBar).XData, [handles.bars(iBar).XOffset]');...
                handles.bars(iBar).XData;...
                handles.bars(iBar).XData]; % line 2 is useless but we need the index to be on line 3
            % to be used by 'xpos(:,i) = mean(x([1 3],:));' given that for
            % 2012 version of Matlab the index are on line 3...
            %             xpos(:,i) = mean(x([1 3],:));
            for iGrp = 1:numgroups
                xpos(iGrp,iBar) = x(1,iGrp);
            end
        else
            warning('please check which script line is to be used in your version of Matlab to get the x axis, before using this function.');
            return;
        end
		
		handles.errors(iBar) = errorbar(xpos(:,iBar), barvalues(:,iBar), errors(:,iBar), 'r', 'linestyle', 'none', 'linewidth', 2);
		ymax = max([ymax; barvalues(:,iBar)+errors(:,iBar)]);
	end
...
% 	ylim([0 ymax*1.1]); % NicoC change: define ylim manually is better
	xlim([0.5 numgroups-change_axis+0.5]);
	
	if strcmp(legend_type, 'axis')
		for iBar = 1:numbars
			xdata = get(handles.errors(iBar),'xdata');
			for j = 1:length(xdata)
				text(xdata(j),  -0.03*ymax*1.1, bw_legend(iBar), 'Rotation', 60, 'fontsize', 12, 'HorizontalAlignment', 'right');
			end
		end

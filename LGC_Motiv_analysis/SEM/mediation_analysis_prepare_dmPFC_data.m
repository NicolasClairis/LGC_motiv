...
%% create matrix
mtrx_THE = [dmPFC_Lac, dmPFC_fMRI, THE, PHE, MHE, kEp, kEm];
% save data to load in R
save([SEM_path,filesep,'dmPFCLac_',ROI_short_Nm,...
...
% con_nm = 'EpEm_Ech';
% NS = 63;
% load([SEM_path,filesep,'dmPFCLac_',ROI_short_Nm,...
%     '_GLM',GLM_str{1},'_',con_nm,'_',num2str(NS),'subs.mat'],'mtrx_THE');
% dmPFC_Lac=mtrx_THE(:,1);
% dmPFC_fMRI=mtrx_THE(:,2);
% THE=mtrx_THE(:,3);
% PHE=mtrx_THE(:,4);
% MHE=mtrx_THE(:,5);
% kEp=mtrx_THE(:,6);
...
goodSubs_bis = ~isnan(dmPFC_Lac_clean).*~isnan(dmPFC_fMRI_clean).*~isnan(THE_clean) == 1;

mtrx_THE_no_outliers = [dmPFC_Lac(goodSubs_bis),...
...
goodSubs_bis = ~isnan(dmPFC_Lac_clean).*~isnan(dmPFC_fMRI_clean).*~isnan(PHE_clean) == 1;

mtrx_PHE_no_outliers = [dmPFC_Lac(goodSubs_bis),...
    dmPFC_fMRI(goodSubs_bis),...
    PHE(goodSubs_bis)];
NS_no_outliers = sum(goodSubs_bis);
% save data to load in R
...
goodSubs_bis = ~isnan(dmPFC_Lac_clean).*~isnan(dmPFC_fMRI_clean).*~isnan(MHE_clean) == 1;

mtrx_MHE_no_outliers = [dmPFC_Lac(goodSubs_bis),...
    dmPFC_fMRI(goodSubs_bis),...
    MHE(goodSubs_bis)];
NS_no_outliers = sum(goodSubs_bis);
% save data to load in R
...
goodSubs_bis = ~isnan(dmPFC_Lac_clean).*~isnan(dmPFC_fMRI_clean).*~isnan(kEp_clean) == 1;
boxcox_kEp_clean = boxcox(kEp);

mtrx_kEp_no_outliers = [dmPFC_Lac(goodSubs_bis),...
    dmPFC_fMRI(goodSubs_bis),...
    kEp(goodSubs_bis),...

...
ROI_short_nm1 = inputdlg(['ROI short name for ',ROI_names{1},'?']);
ROI_short_nm2 = inputdlg(['ROI short name for ',ROI_names{2},'?']);
...
which_prm_ROI1 = listdlg('PromptString',['please select parameter for ',ROI_short_nm1],...
    'ListString',potential_ROI1_prm);
ROI1_prm_nm = potential_ROI1_prm{which_prm_ROI1};
which_prm_ROI2 = listdlg('PromptString',['please select parameter for ',ROI_short_nm2],...
...
[m_betas.ROI2_vs_ROI1_bis,~,pval] = glmfit(betas_ROI1(2,:),...
    betas_ROI2(2,:),...
    'normal');
...
fig2 = fig;
hold on;
hdl = errorbar(m_betas.ROI1,...
    m_betas.ROI2,...
    sem_betas.ROI2);
hdl_fit = plot(m_betas.ROI1,...
    betas_ROI2_vs_ROI1_bis_fit,...
    'LineStyle','--',...
    'LineWidth',lWidth,...
    'Color','k');
hdl.LineWidth = lWidth;

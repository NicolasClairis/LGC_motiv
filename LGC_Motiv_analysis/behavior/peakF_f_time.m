function[b_peakF_perEch_f_time, pval, b_peakF_allE_f_time] = peakF_f_time(study_nm, subject_id, condition, figDisp)
%[b_peakF_perEch_f_time, pval, b_peakF_allE_f_time] = peakF_f_time(study_nm, subject_id, condition, figDisp)
% peakF_f_time will look, for each effort level, how the peak of force
% decreases over time
%
% INPUTS
...
%
% OUTPUTS
% b_peakF_perEch_f_time: nb effort level chosen*NS matrix with slope for
% each effort level chosen per subject
%
% pval: structure with p.value for the slope for each level of effort
% chosen
%
% b_peakF_allE_f_time: 1*NS matrix with slope across all efforts per subject
...
b_peakF_perEch_f_time = NaN(n_Ech_lvl, NS);
b_peakF_allE_f_time = NaN(1,NS);
...
for iS = 1:NS
    ...
    b_peakF_Sub = NaN(2,n_Ech_lvl,2);
    b_peakF_allE_Sub = NaN(2,2);
    [forcePeak_tmp.run1, trialN_tmp.run1, forcePeak_fit_tmp.run1, ...
        forcePeak_tmp.run2, trialN_tmp.run2, forcePeak_fit_tmp.run2] = deal(NaN(1,n_trialBins,n_Ech_lvl));
    [forcePeak_allE_tmp.run1, trialN_allE_tmp.run1, forcePeak_fit_allE_tmp.run1, ...
        forcePeak_allE_tmp.run2, trialN_allE_tmp.run2, forcePeak_fit_allE_tmp.run2] = deal(NaN(1,n_trialBins));
    for iRun = 1:runs.nb_runs.Ep
        ...
        for iEch = Ech_levels
            jEch = iEch + 1;
            Ech_idx = E_chosen_tmp == iEch;
            betas_tmp = glmfit(trialN(Ech_idx), peakF_tmp.allTrials(Ech_idx),'normal');
            b_peakF_Sub(:,jEch,iRun) = betas_tmp;
            peakF_fit_tmp = glmval(betas_tmp, trialN(Ech_idx), 'identity');
            % extract bins
            [forcePeak_tmp.(run_nm_bis)(1,:,jEch),...
                trialN_tmp.(run_nm_bis)(1,:,jEch)] = do_bin2(peakF_tmp.allTrials(Ech_idx), trialN(Ech_idx), n_trialBins,0);
    ...
    % across runs
    b_peakF_perEch_f_time(:,iS) = mean(b_peakF_Sub(2,:,:),3,'omitnan');
    for iEch = Ech_levels
        Ech_nm = ['Ech',num2str(iEch)];
        jEch = iEch + 1;
        forcePeak_f_time.(Ech_nm)(:,iS) = mean([forcePeak_tmp.run1(1,:,jEch); forcePeak_tmp.run2(1,:,jEch)],1,'omitnan');
        trialN_f_time.(Ech_nm)(:,iS) = mean([trialN_tmp.run1(1,:,jEch); trialN_tmp.run2(1,:,jEch)],1,'omitnan');
        forcePeak_fit_f_time.(Ech_nm)(:,iS) = mean([forcePeak_fit_tmp.run1(1,:,jEch); forcePeak_fit_tmp.run2(1,:,jEch)],1,'omitnan');
    end
    b_peakF_allE_f_time(1,iS) = mean(b_peakF_allE_Sub(2,:),2,'omitnan');
    forcePeak_f_time.allE(:,iS) = mean([forcePeak_allE_tmp.run1(1,:); forcePeak_allE_tmp.run2(1,:)],1,'omitnan');
    trialN_f_time.allE(:,iS) = mean([trialN_allE_tmp.run1(1,:); trialN_allE_tmp.run2(1,:)],1,'omitnan');
    forcePeak_fit_f_time.allE(:,iS) = mean([forcePeak_fit_allE_tmp.run1(1,:); forcePeak_fit_allE_tmp.run2(1,:)],1,'omitnan');
end % subject loop
...
for iEch = Ech_levels
    ...
    jEch = iEch + 1;
    [~,pval.(Ech_nm)] = ttest(b_peakF_perEch_f_time(jEch, :));
end
% all efforts 
...
[~,pval.allE] = ttest(b_peakF_allE_f_time);

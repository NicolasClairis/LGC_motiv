function[successSpeed, n_correct, n_errors, RT_avg,...
    ...
    efficacy_bis_pureNback, latency,...
    efficacy_ter_with2first,...
    efficacy_ter_pureNback,...
    RTtotal_with2firstUseless2,...
    RTtotal_pureNback2] = extract_mental_perf(subBehaviorFolder, sub_nm, run_nm)
...
%
% efficacy_bis_with2first: efficacy expressed as
% (n_correct)/RT_total_with2firstUseless
%
% efficacy_bis_pureNback: efficacy expressed as
% (n_correct)/RTtotal_pureNback
%
% latency: time it took to give the first answer to the first digit
% appearing on the screen
%
% efficacy_ter_with2first: efficacy expressed as
% (n_correct-n_errors)/RT_total_with2firstUseless but RTtotal_pureNback extracted
% slightly differently to match what was used in the computational model
%
% efficacy_ter_pureNback: efficacy expressed as
% (n_correct)/RTtotal_pureNback extracted
% slightly differently to match what was used in the computational model
%
% RTtotal_with2firstUseless2: time spent for effort exertion including the
% time spent solving the two first useless answers (like
% RTtotal_with2firstUseless but using slightly different metrics so that
% values are slightly different, but this is the one used for the
% computational model)
%
% RTtotal_pureNback2: time spent for effort exertion ignoring the time spent
% at the beginning for the two first useless answers (like
% RTtotal_pureNback but using slightly different metrics so that
% values are slightly different, but this is the one used for the
% computational model)
%
...
[successSpeed.allTrials,...
    ...
    RTtotal_with2firstUseless2.allTrials,...
    RTtotal_pureNback2.allTrials,...
    efficacy_with2first.allTrials,...
    efficacy_pureNback.allTrials,...
    efficacy_bis_with2first.allTrials,...
    efficacy_bis_pureNback.allTrials,...
    latency.allTrials,...
    efficacy_ter_with2first.allTrials,...
    efficacy_ter_pureNback.allTrials] = deal(NaN(1,nTrialsPerRun));
[successSpeed.per_Ech,...
    ...
    RTtotal_with2firstUseless2.per_Ech,...
    RTtotal_pureNback2.per_Ech,...
    efficacy_with2first.per_Ech,...
    efficacy_pureNback.per_Ech,...
    efficacy_bis_with2first.per_Ech,...
    efficacy_bis_pureNback.per_Ech,...
    efficacy_ter_with2first.per_Ech,...
    efficacy_ter_pureNback.per_Ech,...
...
[successSpeed.per_hE.choice_lowE,...
    ...
    RTtotal_with2firstUseless2.per_hE.choice_lowE,...
    RTtotal_pureNback2.per_hE.choice_lowE,...
    efficacy_with2first.per_hE.choice_lowE,...
    efficacy_pureNback.per_hE.choice_lowE,...
    efficacy_bis_with2first.per_hE.choice_lowE,...
    efficacy_bis_pureNback.per_hE.choice_lowE,...,...
    efficacy_ter_with2first.per_hE.choice_lowE,...
    efficacy_ter_pureNback.per_hE.choice_lowE,...,...
    ...
    RTtotal_with2firstUseless2.per_hE.choice_highE,...
    RTtotal_pureNback2.per_hE.choice_highE,...
    efficacy_with2first.per_hE.choice_highE,...
    efficacy_pureNback.per_hE.choice_highE,...
    efficacy_bis_with2first.per_hE.choice_highE,...
    efficacy_bis_pureNback.per_hE.choice_highE,...
    efficacy_ter_with2first.per_hE.choice_highE,...
    efficacy_ter_pureNback.per_hE.choice_highE,...
...
for iTrial = 1:nTrialsPerRun
    ...
    if ~isempty(onset_names_tmp) && sum(strcmp(onset_names_tmp,'nb_3')) == 1
        ...
        if ~isempty(behaviorStruct.summary.perfSummary{1,iTrial}.rt) % filter trials where no answer was provided
            RTtotal_with2firstUseless2.allTrials(iTrial) = sum(behaviorStruct.summary.perfSummary{1,iTrial}.rt,'omitnan');
            if (length(onset_names_tmp) < 3) % filter trials where no N-back performed
                RTtotal_pureNback2.allTrials(iTrial) = NaN;
            else % otherwise ignore 2 first useless and consider only time afterwards
                RTtotal_pureNback2.allTrials(iTrial) = sum(behaviorStruct.summary.perfSummary{1,iTrial}.rt(3:end),'omitnan');
            end
        else % no answer provided
            RTtotal_with2firstUseless2.allTrials(iTrial) = NaN;
            RTtotal_pureNback2.allTrials(iTrial) = NaN;
        end
    end
    %% extract efficacy
    if (length(onset_names_tmp) < 3) || (n_correct.allTrials(iTrial) < 0) % shitty performance
        ...
        efficacy_bis_with2first.allTrials(iTrial) = 0;
        efficacy_bis_pureNback.allTrials(iTrial) = 0;
        efficacy_ter_with2first.allTrials(iTrial) = 0;
        efficacy_ter_pureNback.allTrials(iTrial) = 0;
    else
        ...
        efficacy_bis_with2first.allTrials(iTrial) = n_correct.allTrials(iTrial)./RTtotal_with2firstUseless.allTrials(iTrial);
        efficacy_bis_pureNback.allTrials(iTrial) = n_correct.allTrials(iTrial)./RTtotal_pureNback.allTrials(iTrial);
        efficacy_ter_with2first.allTrials(iTrial) = n_correct.allTrials(iTrial)./RTtotal_with2firstUseless2.allTrials(iTrial);
        efficacy_ter_pureNback.allTrials(iTrial) = n_correct.allTrials(iTrial)./RTtotal_pureNback2.allTrials(iTrial);
    end
    % extract latency for first reply
    if ~isempty(behaviorStruct.summary.perfSummary{1,iTrial}.rt)
        latency.allTrials(iTrial) = behaviorStruct.summary.perfSummary{1,iTrial}.rt(1,1);
    end
end % trial loop
...
for iEff = 1:n_hE_levels
    ...
    RTtotal_with2firstUseless2.per_hE.choice_lowE(iEff) = mean(RTtotal_with2firstUseless2.allTrials(j_hE_lowEChoice),2,'omitnan');
    RTtotal_pureNback2.per_hE.choice_lowE(iEff) = mean(RTtotal_pureNback2.allTrials(j_hE_lowEChoice),2,'omitnan');
    ...
    efficacy_bis_with2first.per_hE.choice_lowE(iEff) = mean(efficacy_bis_with2first.allTrials(j_hE_lowEChoice),2,'omitnan');
    efficacy_bis_pureNback.per_hE.choice_lowE(iEff) = mean(efficacy_bis_pureNback.allTrials(j_hE_lowEChoice),2,'omitnan');
    efficacy_ter_with2first.per_hE.choice_lowE(iEff) = mean(efficacy_ter_with2first.allTrials(j_hE_lowEChoice),2,'omitnan');
    efficacy_ter_pureNback.per_hE.choice_lowE(iEff) = mean(efficacy_ter_pureNback.allTrials(j_hE_lowEChoice),2,'omitnan');
    ...
    RTtotal_with2firstUseless2.per_hE.choice_highE(iEff) = mean(RTtotal_with2firstUseless2.allTrials(j_hE_hEChoice),2,'omitnan');
    RTtotal_pureNback2.per_hE.choice_highE(iEff) = mean(RTtotal_pureNback2.allTrials(j_hE_hEChoice),2,'omitnan');
    ...
    efficacy_bis_with2first.per_hE.choice_highE(iEff) = mean(efficacy_bis_with2first.allTrials(j_hE_hEChoice),2,'omitnan');
    efficacy_bis_pureNback.per_hE.choice_highE(iEff) = mean(efficacy_bis_pureNback.allTrials(j_hE_hEChoice),2,'omitnan');
    efficacy_ter_with2first.per_hE.choice_highE(iEff) = mean(efficacy_ter_with2first.allTrials(j_hE_hEChoice),2,'omitnan');
    efficacy_ter_pureNback.per_hE.choice_highE(iEff) = mean(efficacy_ter_pureNback.allTrials(j_hE_hEChoice),2,'omitnan');
...
for iEch = 1:n_Ech
    ...
    RTtotal_with2firstUseless2.per_Ech(iEch) = mean(RTtotal_with2firstUseless2.allTrials(jEch_trials),2,'omitnan');
    RTtotal_pureNback2.per_Ech(iEch) = mean(RTtotal_pureNback2.allTrials(jEch_trials),2,'omitnan');
    ...
    efficacy_bis_with2first.per_Ech(iEch) = mean(efficacy_bis_with2first.allTrials(jEch_trials),2,'omitnan');
    efficacy_bis_pureNback.per_Ech(iEch) = mean(efficacy_bis_pureNback.allTrials(jEch_trials),2,'omitnan');
    efficacy_ter_with2first.per_Ech(iEch) = mean(efficacy_ter_with2first.allTrials(jEch_trials),2,'omitnan');
    efficacy_ter_pureNback.per_Ech(iEch) = mean(efficacy_ter_pureNback.allTrials(jEch_trials),2,'omitnan');

...
for iT = 1:nTasks
    ...
    subplot(1,2,iT);
    hold on;
    if sum(low_prm_subs) == sum(high_prm_subs)
    ...
    elseif sum(low_prm_subs) == sum(high_prm_subs) + 1 % in case median split on number that is not odd
        choice_violin_mtrx = [choice_hE.(task_nm).(['low_',prm_nm])(1,:)',...
            [choice_hE.(task_nm).(['high_',prm_nm])(1,:),NaN]',...
            choice_hE.(task_nm).(['low_',prm_nm])(2,:)',...
            [choice_hE.(task_nm).(['high_',prm_nm])(2,:),NaN]',...
            choice_hE.(task_nm).(['low_',prm_nm])(3,:)',...
            [choice_hE.(task_nm).(['high_',prm_nm])(3,:),NaN]'];
    end

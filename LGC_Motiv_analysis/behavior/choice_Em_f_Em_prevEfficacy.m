...
n_bins = 9;
[choice_f_prevEfficacy_with2first, prevEfficacy_f_prevEfficacy_with2first,...
    choice_f_prevEfficacy_pureNback, prevEfficacy_f_prevEfficacy_pureNback,...
    choice_f_trialN,...
...
[r_corr.perSub.choice_f_prevEfficacy_with2first,...
    r_corr.perSub.choice_f_prevEfficacy_pureNback,...
    pval.perSub.choice_f_prevEfficacy_with2first,...
    pval.perSub.choice_f_prevEfficacy_pureNback] = deal(NaN(1, NS));
...
for iS = 1:NS
    ...
    [RT_avg_run_with2first_tmp,...
        ...
        prevEfficacy_avg_run_with2first_tmp,...
        prevEfficacy_avg_run_pureNback_tmp,...
        choice_hE_run_tmp,...
    ...
    for iRun = 1:nTotalRuns
        ...
        if runToInclude == 1
            ...
            for iTrial = 1:nTrialsPerRun
                ...
                if iTrial == 1
                    prevEfficacy_avg_run_with2first_tmp(jTrial) = NaN;
                    prevEfficacy_avg_run_pureNback_tmp(jTrial) = NaN;
                else
                    prevEfficacy_avg_run_with2first_tmp(jTrial) = efficacy_avg_run_with2first_tmp(jTrial - 1);
                    prevEfficacy_avg_run_pureNback_tmp(jTrial) = efficacy_avg_run_with2first_tmp(jTrial - 1);
                end
            end % trial loop
        end % run to include?
    end % run loop
    ...
    okTrials_2first_tmp = ~isnan(choice_hE_run_tmp.*prevEfficacy_avg_run_with2first_tmp);
    okTrials_Nback_tmp = ~isnan(choice_hE_run_tmp.*prevEfficacy_avg_run_pureNback_tmp);
    [r_corr.perSub.choice_f_prevEfficacy_with2first(iS),...
        pval.perSub.choice_f_prevEfficacy_with2first(iS)] = corr(choice_hE_run_tmp(okTrials_2first_tmp), prevEfficacy_avg_run_with2first_tmp(okTrials_2first_tmp));
    [r_corr.perSub.choice_f_prevEfficacy_pureNback(iS),...
        pval.perSub.choice_f_prevEfficacy_pureNback(iS)] = corr(choice_hE_run_tmp(okTrials_Nback_tmp), prevEfficacy_avg_run_pureNback_tmp(okTrials_Nback_tmp));
    
    %% average data per subject
    [choice_f_prevEfficacy_with2first(:,iS), prevEfficacy_f_prevEfficacy_with2first(:,iS)] = do_bin2(choice_hE_run_tmp, prevEfficacy_avg_run_with2first_tmp, n_bins,0);
    [choice_f_prevEfficacy_pureNback(:,iS), prevEfficacy_f_prevEfficacy_pureNback(:,iS)] = do_bin2(choice_hE_run_tmp, prevEfficacy_avg_run_pureNback_tmp, n_bins,0);
...
[r_corr.aSubs.choice_f_prevEfficacy_with2first.mean,...
    r_corr.aSubs.choice_f_prevEfficacy_with2first.sem] = mean_sem_sd(r_corr.perSub.choice_f_prevEfficacy_with2first,2);
[r_corr.aSubs.choice_f_prevEfficacy_pureNback.mean,...
    r_corr.aSubs.choice_f_prevEfficacy_pureNback.sem] = mean_sem_sd(r_corr.perSub.choice_f_prevEfficacy_pureNback,2);
% test if correlation coefficients are significantly different from zero
[~,pval.aSubs.choice_f_prevEfficacy_with2first] = ttest(r_corr.perSub.choice_f_prevEfficacy_with2first);
[~,pval.aSubs.choice_f_prevEfficacy_pureNback] = ttest(r_corr.perSub.choice_f_prevEfficacy_pureNback);
% f efficacy
[choice_f_prevEfficacy_with2first_avg,...
    choice_f_prevEfficacy_with2first_sem] = mean_sem_sd(choice_f_prevEfficacy_with2first,2);
[prevEfficacy_f_prevEfficacy_with2first_avg,...
    prevEfficacy_f_prevEfficacy_with2first_sem] = mean_sem_sd(prevEfficacy_f_prevEfficacy_with2first,2);
[choice_f_prevEfficacy_pureNback_avg,...
    choice_f_prevEfficacy_pureNback_sem] = mean_sem_sd(choice_f_prevEfficacy_pureNback,2);
[prevEfficacy_f_prevEfficacy_pureNback_avg,...
    prevEfficacy_f_prevEfficacy_pureNback_sem] = mean_sem_sd(prevEfficacy_f_prevEfficacy_pureNback,2);
% f trial number
[choice_f_trialN_avg,...
...
fig;
hold on;
hdl = errorbar(prevEfficacy_f_prevEfficacy_with2first_avg,...
    choice_f_prevEfficacy_with2first_avg,...
    choice_f_prevEfficacy_with2first_sem);
hdl.LineWidth = lWidth;
xlabel('Efficacy (with 2 first included)');
ylabel('Choice hE (%)');
legend_size(pSize);
place_r_and_pval(r_corr.aSubs.choice_f_prevEfficacy_with2first.mean,...
    pval.aSubs.choice_f_prevEfficacy_with2first);

%% choice = f(efficacy) only with relevant times
fig;
hold on;
hdl = errorbar(prevEfficacy_f_prevEfficacy_pureNback_avg,...
    choice_f_prevEfficacy_pureNback_avg,...
    choice_f_prevEfficacy_pureNback_sem);
hdl.LineWidth = lWidth;
xlabel('Efficacy');
ylabel('Choice hE (%)');
legend_size(pSize);
place_r_and_pval(r_corr.aSubs.choice_f_prevEfficacy_pureNback.mean,...
    pval.aSubs.choice_f_prevEfficacy_pureNback);

...
%
% Similar to pca_fatigue_brain_and_blood_metabolites but allows you to
% select manually the variables to include in the PCA rather than
% automatically filtering on NaNs
...
all_possible_pca_vars = [fatigue_measures.MPSTEFS_physical_energy',...
...
all_possible_pca_var_nm = {'MPSTEFS_physical_energy',...
...
n_possible_vars = size(all_possible_pca_vars,2);

%% manually select which variables to keep for the PCA
okvars_idx = listdlg('PromptString','Please select variables to include in the PCA',...
    'ListString',all_possible_pca_var_nm);
n_vars_filtered = length(okvars_idx);
pca_vars_filtered = all_possible_pca_vars(:,okvars_idx);
pca_var_nm_filtered = all_possible_pca_var_nm(okvars_idx);

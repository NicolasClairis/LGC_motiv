...
disp(['kEp dmPFC-',metab_nm_bis,': ',...
    'r(c) = ',num2str(round(stats.no_outliers.dmPFC.(metab_nm).kEp.r.c,3)),...
    '; p(c) =  ',num2str(round(pval.no_outliers.dmPFC.(metab_nm).kEp.c,3))]);
disp(['kEp dmPFC-',metab_nm_bis,': ',...
    'r(a) = ',num2str(round(stats.no_outliers.dmPFC.(metab_nm).kEp.r.a,3)),...
    '; p(a) =  ',num2str(round(pval.no_outliers.dmPFC.(metab_nm).kEp.a,3))]);
disp(['kEp dmPFC-',metab_nm_bis,': ',...
    'r(b) = ',num2str(round(stats.no_outliers.dmPFC.(metab_nm).kEp.r.b,3)),...
    '; p(b) =  ',num2str(round(pval.no_outliers.dmPFC.(metab_nm).kEp.b,3))]);
disp(['kEp dmPFC-',metab_nm_bis,': ',...
    'r(c'') = ',num2str(round(stats.no_outliers.dmPFC.(metab_nm).kEp.r.c_prime,3)),...
    '; p(c'') =  ',num2str(round(pval.no_outliers.dmPFC.(metab_nm).kEp.c_prime,3))]);
% kEm
disp(['kEm dmPFC-',metab_nm_bis,': ',...
    'r(c) = ',num2str(round(stats.no_outliers.dmPFC.(metab_nm).kEm.r.c,3)),...
    '; p(c) =  ',num2str(round(pval.no_outliers.dmPFC.(metab_nm).kEm.c,3))]);
disp(['kEm dmPFC-',metab_nm_bis,': ',...
    'r(a) = ',num2str(round(stats.no_outliers.dmPFC.(metab_nm).kEm.r.a,3)),...
    '; p(a) =  ',num2str(round(pval.no_outliers.dmPFC.(metab_nm).kEm.a,3))]);
disp(['kEm dmPFC-',metab_nm_bis,': ',...
    'r(b) = ',num2str(round(stats.no_outliers.dmPFC.(metab_nm).kEm.r.b,3)),...
    '; p(b) =  ',num2str(round(pval.no_outliers.dmPFC.(metab_nm).kEm.b,3))]);
disp(['kEm dmPFC-',metab_nm_bis,': ',...
    'r(c'') = ',num2str(round(stats.no_outliers.dmPFC.(metab_nm).kEm.r.c_prime,3)),...
    '; p(c'') =  ',num2str(round(pval.no_outliers.dmPFC.(metab_nm).kEm.c_prime,3))]);

%% save results automatically
savePath = fullfile('P:','boulot','postdoc_CarmenSandi','results','ROI');
file_nm = [savePath,filesep,'GLM',num2str(GLM),'_',...
    ROI_coords.ROI_nm.ROI_1_shortName,'_ROI_',num2str(NS),'subs_prm.mat'];

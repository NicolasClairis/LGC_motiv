...
%     study_nm, subject_id, condition, GLM,...
%     dmPFC_ROI_infos);
ROI_trialPerTrial = load('ROI_BOLDperTrial_GLM94_65subs_VS_dmPFC.mat',...
...
for iT = 1:nTasks
    ...
    [betas.linear_mdl.(task_nm).b0,...
        ...
        betas.sigmo_mdl.(task_nm).bdmPFC,...
        pval.linear_mdl.(task_nm).b0,...
        pval.linear_mdl.(task_nm).bVS,...
        pval.linear_mdl.(task_nm).bdmPFC,...
        pval.sigmo_mdl.(task_nm).b0,...
        pval.sigmo_mdl.(task_nm).bVS,...
        pval.sigmo_mdl.(task_nm).bdmPFC] = deal(NaN(1,NS));
end % task loop
...
for iT = 1:nTasks
    task_nm_tmp = task_names{iT};
    [~,pval.linear_mdl.(task_nm).b0] = ttest(betas.linear_mdl.(task_nm_tmp).b0);
    [~,pval.linear_mdl.(task_nm).bVS] = ttest(betas.linear_mdl.(task_nm_tmp).bVS);
    [~,pval.linear_mdl.(task_nm).bdmPFC] = ttest(betas.linear_mdl.(task_nm_tmp).bdmPFC);
end

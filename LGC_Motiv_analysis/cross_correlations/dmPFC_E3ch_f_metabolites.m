% dmPFC_E3ch_f_metabolites will check whether dmPFC for E3 chosen depends
% on the level of metabolites.
...
%% focus on E3
fMRI_E3ch_allSubs = fMRI_ROI.choice_high.hE_level(3,:);

%% test correlation with metabolites
[low_met_subs, high_met_subs, metabolite_nm, MRS_ROI_nm, metabolite_allSubs] = medSplit_metabolites(study_nm, subject_id);
goodSubs = ~isnan(metabolite_allSubs).*~isnan(fMRI_E3ch_allSubs) == 1;
[b_fMRI_E3ch_f_mb, ~, stats_fMRI_E3ch_f_mb] = glmfit(metabolite_allSubs(goodSubs), fMRI_E3ch_allSubs(goodSubs), 'normal');
metabolites_ascOrder = sort(metabolite_allSubs(goodSubs));
fMRI_E3ch_f_mb_fit = glmval(b_fMRI_E3ch_f_mb, metabolites_ascOrder,'identity');
...
if dispFig == true
    ...
    %% fMRI E3 chosen = f(metabolites)
    fig;
    scat_hdl = scatter(metabolite_allSubs(goodSubs), fMRI_E3ch_allSubs(goodSubs));
    scat_hdl.MarkerEdgeColor = 'k';
    scat_hdl.LineWidth = lWidth;
    fit_hdl = plot(metabolites_ascOrder, fMRI_E3ch_f_mb_fit);
    fit_hdl.LineWidth = lWidth;
    fit_hdl.LineStyle = '-';
    fit_hdl.Color = grey;
    xlabel([MRS_ROI_nm,' - ',metabolite_nm]);
    ylabel([fMRI_ROI_short_nm,' E3 chosen']);
    legend_size(pSize);
    ...
    ylabel({[fMRI_ROI_short_nm,' BOLD during ',timePeriod_nm];...
        full_ROI_taskName});
    legend_size(pSize);

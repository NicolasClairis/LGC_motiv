%% script to correlate plasma metabolites with behavioral parameters while correcting for age and sex.
% For the same analysis but without correcting for age and sex, look at 
% parameters_f_plasmaMetabolites.m
%
...
for iUncorrCorr = 1:length(subsIncluded)
    ...
    for iPrm = 1:n_prm
        ...
        if ~strcmp(prm_nm,'CID') % no sense in this case
            ...
            x_vars = [age(goodS_tmp)', sex(goodS_tmp)', metabolite_allSubs(goodS_tmp)'];
            [~, betas_tmp, pval_tmp, ~,...
    ...
    if fig_disp == 1
        ...
        for iPrm = 1:n_prm
            ...
            if ~strcmp(prm_nm,'CID') % no sense in this case
                goodS_tmp = goodSubs.(uncCorr_nm).(prm_nm);
                fig;
                scat_hdl = scatter(metabolite_allSubs(goodS_tmp)', prm.(prm_nm)(goodS_tmp)');

...
[low_kEp_subs, high_kEp_subs] = medSplit_prm(study_nm, subject_id, 'kEp');
[low_kEm_subs, high_kEm_subs] = medSplit_prm(study_nm, subject_id, 'kEm');
...
for iTask = 1:nTasks
    ...
    switch task_nm
        case 'Ep'
            low_kE_subs = low_kEp_subs;
            high_kE_subs = high_kEp_subs;
            prm_nm = 'kEp';
        case 'Em'
            low_kE_subs = low_kEm_subs;
            high_kE_subs = high_kEm_subs;
            prm_nm = 'kEm';
    end
    ...
    [fMRI_ROI.(task_nm).(['l',prm_nm]).hE_level.mean,...
        fMRI_ROI.(task_nm).(['l',prm_nm]).hE_level.sem] = mean_sem_sd(fMRI_ROI.(task_nm).hE_level(:,low_kE_subs),2);
    [fMRI_ROI.(task_nm).(['h',prm_nm]).hE_level.mean,...
        fMRI_ROI.(task_nm).(['h',prm_nm]).hE_level.sem] = mean_sem_sd(fMRI_ROI.(task_nm).hE_level(:,high_kE_subs),2);
    [choice_hE.(task_nm).(['l',prm_nm]).hE_level.mean,...
        choice_hE.(task_nm).(['l',prm_nm]).hE_level.sem] = mean_sem_sd(choice_hE.(task_nm).hE_level(:,low_kE_subs),2);
    [choice_hE.(task_nm).(['h',prm_nm]).hE_level.mean,...
        choice_hE.(task_nm).(['h',prm_nm]).hE_level.sem] = mean_sem_sd(choice_hE.(task_nm).hE_level(:,high_kE_subs),2);
    % median split on choices and fMRI for effort chosen
    [fMRI_ROI.(task_nm).(['l',prm_nm]).Ech.mean,...
        fMRI_ROI.(task_nm).(['l',prm_nm]).Ech.sem] = mean_sem_sd(fMRI_ROI.(task_nm).Ech(:,low_kE_subs),2);
    [fMRI_ROI.(task_nm).(['h',prm_nm]).Ech.mean,...
        fMRI_ROI.(task_nm).(['h',prm_nm]).Ech.sem] = mean_sem_sd(fMRI_ROI.(task_nm).Ech(:,high_kE_subs),2);
    % median split on fMRI for effort level and choice made
    [fMRI_ROI.(task_nm).(['l',prm_nm]).choice_low.hE_level.mean,...
        fMRI_ROI.(task_nm).(['l',prm_nm]).choice_low.hE_level.sem] = mean_sem_sd(fMRI_ROI.(task_nm).choice_low.hE_level(:,low_kE_subs),2);
    [fMRI_ROI.(task_nm).(['l',prm_nm]).choice_high.hE_level.mean,...
        fMRI_ROI.(task_nm).(['l',prm_nm]).choice_high.hE_level.sem] = mean_sem_sd(fMRI_ROI.(task_nm).choice_high.hE_level(:,low_kE_subs),2);
    [fMRI_ROI.(task_nm).(['h',prm_nm]).choice_low.hE_level.mean,...
        fMRI_ROI.(task_nm).(['h',prm_nm]).choice_low.hE_level.sem] = mean_sem_sd(fMRI_ROI.(task_nm).choice_low.hE_level(:,high_kE_subs),2);
    [fMRI_ROI.(task_nm).(['h',prm_nm]).choice_high.hE_level.mean,...
        fMRI_ROI.(task_nm).(['h',prm_nm]).choice_high.hE_level.sem] = mean_sem_sd(fMRI_ROI.(task_nm).choice_high.hE_level(:,high_kE_subs),2);

    % betas
    % global betas with effort chosen
    b_fMRI_f_Ech.(task_nm).(['low_',prm_nm]) = b_fMRI_f_Ech.(task_nm).allSubs(:,low_kE_subs);
    b_fMRI_f_Ech.(task_nm).(['high_',prm_nm]) = b_fMRI_f_Ech.(task_nm).allSubs(:,high_kE_subs);
    % betas split by effort level and choice
    b_fMRI_f_E.(task_nm).lE_chosen.(['low_',prm_nm]) = b_fMRI_f_E.(task_nm).lE_chosen.allSubs(:,low_kE_subs);
    b_fMRI_f_E.(task_nm).lE_chosen.(['high_',prm_nm]) = b_fMRI_f_E.(task_nm).lE_chosen.allSubs(:,high_kE_subs);
    b_fMRI_f_E.(task_nm).hE_chosen.(['low_',prm_nm]) = b_fMRI_f_E.(task_nm).hE_chosen.allSubs(:,low_kE_subs);
    b_fMRI_f_E.(task_nm).hE_chosen.(['high_',prm_nm]) = b_fMRI_f_E.(task_nm).hE_chosen.allSubs(:,high_kE_subs);

    %% ttest
    [~,pval.(task_nm).choice_hE_vs_lE.allSubs] = ttest(b_fMRI_f_E.(task_nm).lE_chosen.allSubs(2,:),...
        b_fMRI_f_E.(task_nm).hE_chosen.allSubs(2,:));
    [~,pval.(task_nm).choice_hE_vs_choice_hE_slope.(['low_vs_high_',prm_nm])] = ttest2(b_fMRI_f_E.(task_nm).hE_chosen.(['low_',prm_nm])(2,:),...
        b_fMRI_f_E.(task_nm).hE_chosen.(['high_',prm_nm])(2,:));
    ...
    if dispFig == true
        ...
        fMRI_Ech_low = errorbar(Echosen_possible-0.01,...
            fMRI_ROI.(task_nm).(['l',prm_nm]).Ech.mean,...
            fMRI_ROI.(task_nm).(['l',prm_nm]).Ech.sem);
        fMRI_Ech_high = errorbar(Echosen_possible+0.01,...
            fMRI_ROI.(task_nm).(['h',prm_nm]).Ech.mean,...
            fMRI_ROI.(task_nm).(['h',prm_nm]).Ech.sem);
        % curve parameters
        ...
        fMRI_hE_low = errorbar(hE_levels-0.01,...
            fMRI_ROI.(task_nm).(['l',prm_nm]).hE_level.mean,...
            fMRI_ROI.(task_nm).(['l',prm_nm]).hE_level.sem);
        fMRI_hE_high = errorbar(hE_levels+0.01,...
            fMRI_ROI.(task_nm).(['h',prm_nm]).hE_level.mean,...
            fMRI_ROI.(task_nm).(['h',prm_nm]).hE_level.sem);
        % curve parameters
        ...
        choice_hE_low = errorbar(hE_levels-0.01,...
            choice_hE.(task_nm).(['l',prm_nm]).hE_level.mean,...
            choice_hE.(task_nm).(['l',prm_nm]).hE_level.sem);
        choice_hE_high = errorbar(hE_levels+0.01,...
            choice_hE.(task_nm).(['h',prm_nm]).hE_level.mean,...
            choice_hE.(task_nm).(['h',prm_nm]).hE_level.sem);
        % curve parameters
        ...
        fig;
        hold on;
        fMRI_lowEch_lowPrm = errorbar(hE_levels-0.01,...
            fMRI_ROI.(task_nm).(['l',prm_nm]).choice_low.hE_level.mean,...
            fMRI_ROI.(task_nm).(['l',prm_nm]).choice_low.hE_level.sem);
        fMRI_lowEch_highPrm = errorbar(hE_levels+0.01,...
            fMRI_ROI.(task_nm).(['h',prm_nm]).choice_low.hE_level.mean,...
            fMRI_ROI.(task_nm).(['h',prm_nm]).choice_low.hE_level.sem);
        fMRI_highEch_lowPrm = errorbar(hE_levels-0.01,...
            fMRI_ROI.(task_nm).(['l',prm_nm]).choice_high.hE_level.mean,...
            fMRI_ROI.(task_nm).(['l',prm_nm]).choice_high.hE_level.sem);
        fMRI_highEch_highPrm = errorbar(hE_levels+0.01,...
            fMRI_ROI.(task_nm).(['h',prm_nm]).choice_high.hE_level.mean,...
            fMRI_ROI.(task_nm).(['h',prm_nm]).choice_high.hE_level.sem);
        % curve parameters
        fMRI_lowEch_lowPrm.LineStyle = '--';
        fMRI_lowEch_lowPrm.LineWidth = lWidth;
        fMRI_lowEch_lowPrm.Color = blue;
        fMRI_lowEch_highPrm.LineStyle = '-';
        fMRI_lowEch_highPrm.LineWidth = lWidth;
        fMRI_lowEch_highPrm.Color = blue;
        fMRI_highEch_lowPrm.LineStyle = '--';
        fMRI_highEch_lowPrm.LineWidth = lWidth;
        fMRI_highEch_lowPrm.Color = purple;
        fMRI_highEch_highPrm.LineStyle = '-';
        fMRI_highEch_highPrm.LineWidth = lWidth;
        fMRI_highEch_highPrm.Color = purple;
        legend([fMRI_highEch_highPrm, fMRI_highEch_lowPrm,...
            fMRI_lowEch_highPrm, fMRI_lowEch_lowPrm],...
            {['high ',prm_nm, '- high E chosen'],['low ', prm_nm, '- high E chosen'],...
            ['high ',prm_nm, '- low E chosen'],['low ', prm_nm, '- low E chosen']});
        legend('Location','NorthWest');
        legend('boxoff');
        xticks(hE_levels);
        xlabel('high Effort level');

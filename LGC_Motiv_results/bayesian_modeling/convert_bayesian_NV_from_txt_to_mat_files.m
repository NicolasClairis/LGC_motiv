...
[bayesian_NV.mdl1.subject_id] = convert_sub_id_from_num_to_cell(sub_id_line1);
NS = length(bayesian_NV.mdl1.subject_id);
for iS = 1:NS
    sub_nm = bayesian_NV.mdl1.subject_id{iS};
    for iRun = 1:nRuns
        run_nm = ['run',num2str(iRun)];
        trial_idx = (1:nTrialsPerRun) + nTrialsPerRun*(iRun - 1);
        bayesian_NV.mdl1.(['CID',sub_nm]).(run_nm) = NV_data_mdl1(trial_idx);
    end % run loop
end % subject loop
...
[bayesian_NV.mdl2.subject_id] = convert_sub_id_from_num_to_cell(sub_id_line2);
NS = length(bayesian_NV.mdl2.subject_id);
for iS = 1:NS
    sub_nm = bayesian_NV.mdl2.subject_id{iS};
    for iRun = 1:nRuns
        run_nm = ['run',num2str(iRun)];
        trial_idx = (1:nTrialsPerRun) + nTrialsPerRun*(iRun - 1);
        bayesian_NV.mdl2.(run_nm) = NV_data_mdl2(trial_idx);
    end % run loop
end % subject loop

%% make one global save
save([gitPath, 'bayesian_NV_data.mat'],'bayesian_NV');
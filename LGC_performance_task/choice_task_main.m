...
%% working directories
scripts_folderName = 'LGC_performance_task';
...
[iSubject, effort_type, session_nm, visit_nm] = deal([]);
while isempty(iSubject) || length(iSubject) ~= 3 ||...
        isempty(effort_type) || ~ismember(effort_type,{'p','m'}) ||...
        isempty(session_nm) || str2double(session_nm) < 0 ||...
        isempty(visit_nm) || ~ismember(visit_nm,1:3) % repeat until both are answered
    info = inputdlg({'Subject CID (XXX)','Type d''effort (p/m)','Session number(0-4) (0 for initial calibration)','visit (1-3)'});
    [iSubject, effort_type, session_nm, visit_nm] = info{[1, 2, 3, 4]};
end
session_nb = str2double(session_nm);
visit_nb = str2double(visit_nm);
switch effort_type
    case 'p'
...
% file name
file_nm = [subjectCodeName,'_session',session_nm,'_',effort_type,'_task_visit',visit_nm];
...
trialOptions = trial_options_design(n_R_levels, n_E_levels, punishment_yn, nTrials);

error('define levels of effort here');
switch effort_type
    case 'physical'
...
elseif session_nb > 0
    ...
    if IRM == 1
        ...
        all.choice_opt = trialOptions; % choice options
